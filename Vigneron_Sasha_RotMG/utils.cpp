...
float utils::GetDistance(const Point2f& pos1, const Point2f& pos2) 
{
...
#pragma region OtherUseful
void utils::Print(const std::string& message) 
{
	std::cout << message << std::endl;
}
Point2f utils::ScreenToWorldPoint(const Point2f& mousePos, const Point2f& playerPos, const Window window, float menuWidth)
{
	Point2f worldPoint{ mousePos };
	worldPoint.x += playerPos.x;
	worldPoint.y += playerPos.y;
	worldPoint.x -= (window.width - menuWidth) / 2;
	worldPoint.y -= window.height / 2.f;
	return worldPoint;
}
#pragma endregion	